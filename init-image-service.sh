...
# 기존 서비스 삭제
log "image-service undeploy"
docker rm -f image-service

# 기존 image-service 이미지를 삭제하고 새로 빌드
log "image-service image remove and build."
docker rmi image-service:latest || true
docker build -t image-service:latest .

# Docker로 image-service 서비스 실행
log "Execute image-service..."
docker run -d \
  --name image-service \
